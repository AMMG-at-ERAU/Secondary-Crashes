                 ...
                 cxxcode = FALSE, nthreads = 1)
{
  ...
  if (is.null(param0))
  {
    ...
    param0 <- c(mu=mu0, A=0.01, alpha=1)  # Keep mu, A, alpha, gamma
  ...
  # write.table(rbwd, file="bwd data.csv", row.names=F, sep=",")#change
  
  # check initial values for the model parameters
  if (!is.numeric(param0) || length(param0) != 3 || any(param0 < 0))  #change length to 7
    stop("param0 must be a numeric vector of length 4 with positive components")

  param1 <- param0
  thetar <- asd <- matrix(NA, nrow=no.itr, ncol=3) #change matrix column to 7
  par.names <- c("mu", "A", "alpha") #change
  ...
  for (itr in 1:no.itr)
  {
    ...
    for (l in 1:(no.itr + 1 - itr))
    {
      ...
      revents <- bkg$revents
      #write.table(revents, file="events data.csv", row.names=F, sep=",")#change
      utils::setTxtProgressBar(bkgpbar, l)
    }
    close(bkgpbar)
    integ0 <- bkg$integ0
    dbk <- bk - revents[, 5] #change
    bk <- revents[, 5]  #change
    pb <- revents[, 6]   #change
    if (verbose)
    {
    ...
    if (itr > 1)
    {
      ...
      #if (all(c(dtht, dlrv) < rel.tol))#change
        break
    } else{
      if (itr == no.itr)
  ...
  names(param1) <- c("mu", "A", "alpha") #changed
