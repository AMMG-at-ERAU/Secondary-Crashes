...
catalog <- function(data, time.begin=NULL, study.start=NULL,
                   ...
                   region.poly=NULL,
                   flatmap=TRUE, dist.unit="degree", tz="GMT") #mag removed
{
  data <- as.data.frame(data)
  dnames <- tolower(names(data))
  names(data) <- dnames
  vnames <- c("date", "time", "long", "lat") #remove mag
  ...
  if (!is.numeric(data$lat) || !is.numeric(data$long)) #remove mag
 ...
 # mm <- mm[ok] - mag.threshold
  ...
  revents <- cbind(tt, xx, yy, flag, bkgd=0, prob=1, lambd=0) #removed mm
  longlat.coord <- longlat.coord[ok, ]
  longlat.coord$flag <- flag
  longlat.coord$dt <- dt[ok]
  X <- spatstat::ppx(data.frame(t=tt, x=xx, y=yy),
                     coord.type=c("t", "s", "s"))     #removed mm
  ...
  out <- list(revents=revents, rpoly=rpoly, rtperiod=rtperiod, X=X,
              ...
              dist.unit=dist.unit)   #removed  magnitude
  class(out) <- "catalog"
  return(out)
}
...
print.catalog <- function (x, ...)
{
  ...
  cat("\nnumber of events:\n  total events", nrow(x$revents),
      ":", sum(x$revents[, 4] == 1), "target events, ",
      sum(x$revents[, 4] != 1), "complementary events\n  (",
      sum(x$revents[, 4] == 0), "events outside geographical region,",
      sum(x$revents[, 4] == -2), "events outside study period)\n")
}
