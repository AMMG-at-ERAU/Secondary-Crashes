...
// value of the gaussian kernel function at r with bandwidth sig, Eq 8
double dGauss(double r, double sig)
{
  return exp(-(r * r) /(2 * sig * sig)) / (2 * PI * sig * sig);
}

// integral of the gaussian kernel with bandwidth w[0] from 0 to r needed
// in eq 10
             ...
             SEXP tperiod)
{
  ...
  double t[N], x[N], y[N], bk[N], pb[N], lam[N]; //remove magnitude
  for (int i = 0; i < N; i++)
    {
      t[i] = events[i];
      x[i] = events[N + i];
      y[i] = events[2 * N + i];
 /*      m[i] = events[3 * N + i]; */
      bk[i] = events[4 * N + i];//Changed 5 to 4
      pb[i] = events[5 * N + i];//Changed 6 to 5
      lam[i] = events[6 * N + i];//Changed 7 to 6
    }
...
// Function of x,y
// Compute bandwith kernel estimator Eq 7
...
/*       s = 0;
      for (int j = 0; j < N; j++)
	{
      ...
      bk[i] = 0.7; // Changed
      events[4 * N + i] = bk[i];// Changed 5 to 4
    }
// polyinteg approximate integral function on a polygon region, check poly code
  s = tlength - tstart2;
  for (int i = 0; i < N; i++)
    {
      w[0] = bwd[i];
	  // double integral in eq 10 multiplied by T. Only time integral here
/*       s += pb[i];// Changed */
	  // compute lamda in eq 2
      lam[i] = clambdaj(tht,i, t, x, y, bk); //remove magnitude
	  // compute and store 1-prob in eq 4
      events[5 * N + i] = (tht[0] * tht[0] * bk[i]) / lam[i]; // Changed 6 to 5
	  // store lamda in eq 2
      events[6 * N + i] = lam[i];// Changed 7 to 6
    }
